...
from ml_pipelines.components.model_training import ModelTrainPipeline, ModelTrain
...
def test_create_preprocessor():
    ...
    pipeline_params = {
        'numerical_features': ['days_to_estimation,', 'days_after_carrier,', 'days_to_shipping_limit,', 'shipping_status,',],
...
def test_create_train_test_split():
    ...
    mlflow_tracking_cfg = MlflowTrackingCfg(
        run_name="test_run",
        model_train_experiment_path="/fake/path/to/experiment",
        model_name="test_model"
    )
    feature_store_table_cfg = FeatureStoreTableCfg(
        query_features="SELECT * FROM features_table"
    )
    labels_table_cfg = LabelTableCfg(
        query_target="SELECT * FROM labels_table",
        target="target"
    )
    pipeline_params = {
    ...
    data = [
        Row(order_id=i, num_feature=i * 1.0, cat_feature=str(i % 2), target=i % 2, dt_ref=(datetime(2023, 1, 1) + timedelta(days=i)).strftime('%Y-%m-%d'))
        for i in range(50)
    ]
