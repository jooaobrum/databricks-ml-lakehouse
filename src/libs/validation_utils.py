...
class DataQuality:
    ...
    def _initialize_data_context(self):
        ...
        data_context_configuration = DataContextConfig(
            store_backend_defaults=FilesystemStoreBackendDefaults(
                root_directory=self.save_directory_expectations
            )
        )
    ...
    def generate_profiling_dataframe(self, result):
        ...
        df_profiling = pd.DataFrame(columns=('batch_name', 'batch_time', 'feature', 'missing', 'percentage_missing', 'distinct_values', 'percentage_distinct', 'mean', 'variance', 'max', 'min'))
